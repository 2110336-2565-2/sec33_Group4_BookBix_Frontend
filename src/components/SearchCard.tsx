...
export const SearchCard = () => {
  const LocationCard = () => {
    return (
      ...
      //   {mockup.map((location, idx) => (
      //     <Card>
      //       <Card.Img variant="top" src={location.images[0]} />
      //       <Card.Body>
      //         <Card.Title>{location.name}</Card.Title>
      //         <Card.Text>{location.description}</Card.Text>
      //         <Card.Footer>{location.price}</Card.Footer>
      //       </Card.Body>
      //     </Card>
      //     //</Col>
      //   ))}
      // </Row>
      // make 3 location cards per row
      <Row className="g-4">
        {mockup.map((location, idx) => (
          <Card>
            <a href="https://mdbootstrap.com/docs/react/components/cards/">
              <Card.Img variant="top" className="card-location-img" src={location.images[0]} />
            </a>
            <Card.Body>
              <Card.Title>{location.name}</Card.Title>
              <Card.Text className="text-gray">{location.address.split(',')[0]}</Card.Text>
              {/* <blockquote className="blockquote">
                <footer className="blockquote-footer">{location.address.split(',')[0]}</footer>
              </blockquote> */}
              {/* <Card.Img variant="bottom" className="star" src={star} alt="star" /> */}
            </Card.Body>
            <Card.Body>
              <Card.Text className="">4.5 ⭐️</Card.Text>
              <Card.Text className="bottom-card">Price: {location.price}/hour</Card.Text>
            </Card.Body>
          </Card>
        ))}
      </Row>
